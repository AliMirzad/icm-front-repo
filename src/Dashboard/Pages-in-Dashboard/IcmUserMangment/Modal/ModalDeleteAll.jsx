...
import deleteAllUser from "../../../../Api/icmUserMangmentApi/deleteAllUser";

const ModalDeleteAll = ({id,showDelteModalFn,test}) => {
    console.log(id);
    
    const navigate = useNavigate();
  // const [showDelteModal,setDelteModal]=useState(showDelteModal)
    let [massage,setMassage]=useState("")
    
    
    const deletAllUsersCompany = async () => {
      try {
        let response = await deleteAllUser(id);
        let data = await response;
        
        setMassage(data.responseMessage)
    console.log(test.length+"at the end");
      ...
      <div className={`  inset-0 absolute items-center  text-center bg-black bg-opacity-50 z-[99999] `}>
        ...
        {massage?<h3 className="text-lg font-semibold mb-4 text-red-600">{massage}</h3>:<>
          ...
          {massage?<Link onClick={handleCloseModal} className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors duration-200">
            ...
            <button onClick={deletAllUsersCompany} className="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-colors duration-200">
              بله
            </button>
          </>}
          </div>
